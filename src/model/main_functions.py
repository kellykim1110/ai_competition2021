...
from src.functions.metric import get_score, get_ai_score
    ...
    for epoch in mb:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if args.gradient_accumulation_steps > 1:
                loss[0] = loss[0] / args.gradient_accumulation_steps
                loss[1] = loss[1] / args.gradient_accumulation_steps
            ...
            if (global_step + 1) % 50 == 0:
                ...
                print("{} step processed.. Current Loss : {}".format((global_step+1),loss[1].item()))
        ...
        results = evaluate(args, model, tokenizer, logger, epoch_idx = str(epoch_idx), tr_loss = loss[1].item())


        # model save
        if (args.logging_steps > 0 and max_f1 < float(results["macro_f1_score"])):
            max_f1 = float(results["macro_f1_score"])
            # 모델 저장 디렉토리 생성
...
def evaluate(args, model, tokenizer, logger, epoch_idx = "", tr_loss = 1):
    ...
    results = get_ai_score(pred_labels, gold_labels, idx2label)
...
def predict(args, model, tokenizer):
    ...
    print(confusion_matrix(pred_labels, gold_labels))
    confusion_m = confusion_matrix(pred_labels, gold_labels)
    confusion_list = [[], [0 for i in range(0, len(confusion_m))], []]
    for i in range(0, len(confusion_m)):
        # recall
        confusion_list[2].append(sum(confusion_m[i]))
        for j in range(0, len(confusion_m[i])):
            # precision
            confusion_list[1][j] += confusion_m[i][j]
            if (i == j): confusion_list[0].append(confusion_m[i][j])
    all_cnt = sum([sum(i) for i in confusion_m])
    print("micro_accuracy: "+str(round((sum(confusion_list[0])/all_cnt), 4)))
    precision = sum([i/j for i,j in zip(confusion_list[0], confusion_list[1])])/len(confusion_list[0])
    recall = sum([i / j for i, j in zip(confusion_list[0], confusion_list[2])]) / len(confusion_list[0])
